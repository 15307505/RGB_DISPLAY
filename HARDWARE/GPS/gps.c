...
*********************************************************************************************************
*/
void gps_pro(u8 *ucaGpsBuf,uint16_t usGpsPos)
{
	Analyze0183(ucaGpsBuf, usGpsPos-1);   //  解码GPS协议 NMEA-0183协议
}
...
void Analyze0183(uint8_t *_ucaBuf, uint16_t _usLen)
{
	...
	//if (memcmp(_ucaBuf, "GNGGA,", 6) == 1)
	{
		gpsGPGGA(_ucaBuf, _usLen);
	}
	//else if (memcmp(_ucaBuf, "GPGSA,", 6) == 1)
	{
		gpsGPGSA(_ucaBuf, _usLen);
	}
	//else if (memcmp(_ucaBuf, "GPGSV,", 6) == 1)
	{
		gpsGPGSV(_ucaBuf, _usLen);
	}
	//else if (memcmp(_ucaBuf, "GNRMC,", 6) == 1)
	{
		gpsGPRMC(_ucaBuf, _usLen);
	}
	//else if (memcmp(_ucaBuf, "GNVTG,", 6) == 1)
	{
		gpsGPVTG(_ucaBuf, _usLen);
	}
	//else if (memcmp(_ucaBuf, "GPGLL,", 6) == 1)
	{
...
/*  GPS数据
$GNGGA,035422.000,3131.438603,N,12018.111621,E,1,5,2.50,14.949,M,6.808,M,,*41
$GPGSA,A,3,14,10,32,24,,,,,,,,,2.67,2.50,0.95*09
$GLGSA,A,3,70,,,,,,,,,,,,2.67,2.50,0.95*11
$GPGSV,3,1,10,10,75,284,31,25,49,146,17,12,46,081,18,32,43,316,40*7D
$GPGSV,3,2,10,24,29,046,17,14,25,298,35,31,23,241,,21,08,196,*76
$GPGSV,3,3,10,20,07,149,,193,,,*7A
$GLGSV,2,1,07,80,85,272,,69,56,104,,70,51,343,32,73,30,214,*6C
$GLGSV,2,2,07,79,26,029,25,68,10,129,,71,05,322,*51
$GNRMC,035422.000,A,3131.438603,N,12018.111621,E,0.00,174.46,080218,,,A*75
$GNVTG,174.46,T,,M,0.00,N,0.00,K,A*23
...
/*	
	...
	sprintf((char *)dtbuf,"Longitude:%.5f %1c   ",tp/=100000,gpsx.ewhemi);	//得到经度字符串
 	LCD_ShowString(330,230,200,16,16,dtbuf);	 	   
	tp=gpsx.latitude;	   
	sprintf((char *)dtbuf,"Latitude:%.5f %1c   ",tp/=100000,gpsx.nshemi);	//得到纬度字符串
 	LCD_ShowString(330,250,200,16,16,dtbuf);	 	 
	tp=gpsx.altitude;	   
 	sprintf((char *)dtbuf,"Altitude:%.1fm     ",tp/=10);	    			//得到高度字符串
 	LCD_ShowString(330,270,200,16,16,dtbuf);	 			   
	tp=gpsx.speed;	   
 	sprintf((char *)dtbuf,"Speed:%.3fkm/h     ",tp/=1000);		    		//得到速度字符串	 
 	LCD_ShowString(330,290,200,16,16,dtbuf);	 				    
	if(gpsx.fixmode<=3)														//定位状态
	{  
		sprintf((char *)dtbuf,"Fix Mode:%s",fixmode_tbl[gpsx.fixmode]);	
	  	LCD_ShowString(330,310,200,16,16,dtbuf);			   
	}	 	   
	sprintf((char *)dtbuf,"Valid satellite:%02d",gpsx.posslnum);	 		//用于定位的卫星数
 	LCD_ShowString(330,330,200,16,16,dtbuf);	    
	sprintf((char *)dtbuf,"Visible satellite:%02d",gpsx.svnum%100);	 		//可见卫星数
 	LCD_ShowString(330,350,200,16,16,dtbuf);		 
	sprintf((char *)dtbuf,"UTC Date:%04d/%02d/%02d   ",gpsx.utc.year,gpsx.utc.month,gpsx.utc.date);	//显示UTC日期
	//printf("year2:%d\r\n",gpsx.utc.year);
	LCD_ShowString(330,370,200,16,16,dtbuf);		    
	sprintf((char *)dtbuf,"UTC Time:%02d:%02d:%02d   ",gpsx.utc.hour,gpsx.utc.min,gpsx.utc.sec);	//显示UTC时间
  LCD_ShowString(330,390,200,16,16,dtbuf);		
*/
  sprintf((char *)dtbuf,"Valid satellite:%02d",g_tGPS.ViewNumber);	 		//用于定位的卫星数
 	LCD_ShowString(330,330,200,16,16,dtbuf);	    
	sprintf((char *)dtbuf,"Visible satellite:%02d",g_tGPS.ViewNumber);	 		//可见卫星数
 	LCD_ShowString(330,350,200,16,16,dtbuf);	
